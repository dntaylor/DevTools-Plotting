...
class WTauFakeRateFlattener(NtupleFlattener):
    '''
    WTauFakeRate flattener
    '''

    def __init__(self,sample,**kwargs):
        # setup properties
        self.baseCutMap = {
            'zDR'      : lambda row: row.z_deltaR>0.02,
            'wmPt'     : lambda row: row.m_pt>25,
            'oldDM'    : lambda row: row.t_decayModeFinding,
            'zVeto'    : lambda row: row.z_mass<40 or row.z_mass>100,
            'wmMt'     : lambda row: row.wm_mt>60,
        }
        self.baseCutMapNew = {
            'zDR'      : lambda row: row.z_deltaR>0.02,
            'wmPt'     : lambda row: row.m_pt>25,
        ...
        super(WTauFakeRateFlattener, self).__init__('WTauFakeRate',sample,**kwargs)
    ...
    def getWeight(self,row):
        if row.isData:
            ...
            # per event weights
            base = ['genWeight','pileupWeight','triggerEfficiency','m_tightScale']
...
def main(argv=None):
    ...
    flattener = WTauFakeRateFlattener(
        args.sample,
        shift=args.shift,
    )
